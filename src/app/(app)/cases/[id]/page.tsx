      ...
      <main className="space-y-6">
                  ...
                  {segments.map((s) => (
                    ...
                    >
                      ...
                      <span className="flex items-center gap-2 text-xs text-muted-foreground">
                        {s.status} · {Math.round(s.plannedMinutes / 60)}h
                        planned
                        {s.status !== 'completed' &&
                        s.status !== 'cancelled' &&
                ...
                <ol className="space-y-1 text-xs text-muted-foreground">
                  {assignments.map((a) => (
                    ...
                    >
                      <span>
                        #{a.sequenceNo + 1}{' '}
                        {allWorkshops.find((w) => w.id === a.workshopId)
                          ?.name ?? a.workshopId.slice(0, 8)}{' '}
                        · {a.role}
                      </span>
                      <span>{a.status}</span>
                    </li>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Flag missing parts; the coordinator orders, receives, and
              withdraws. Status reconciles estimated vs ordered vs received.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                Per-kind requirements are validated on save (e.g. insurance
                needs an insurer).
              </p>
            </form>
          </CardContent>
