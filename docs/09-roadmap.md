...
**Status:** 🚧 In progress. Landed so far: (1) **i18n foundation** — Norwegian Bokmål (nb-NO) is the binding default for all user-facing UI (`src/lib/i18n`, type-safe nb/en catalogs, `getDictionary`/`resolveLocale`/`format`); DB schema, code, APIs and internals stay English. (2) **Documents module** (docs/04) — `documents`, `document_links` (polymorphic), `document_access_events` (append-only) with sensitivity-class storage buckets + deterministic tenant-prefixed paths. (3) **Image upload pipeline** — Supabase Storage adapter (signed direct-to-storage upload, on-the-fly thumbnails, `pnpm storage:setup` bucket provisioning); `PhotoUploader` (drag & drop, mobile camera, multiple files, progress) + `PhotoGallery` full-screen viewer, før/under/etter on case detail (Norwegian); graceful metadata-only fallback when storage is absent. (4) **Quality control** — `checklist_templates`/`_items` (per-workshop configurable), `checklist_runs` (status DERIVED at sign-off), `checklist_responses` (comment/photo required on fail, enforced in the service), `quality_deviations` (internal-rework kept separable); QC failure-rate + rework-rate SSoT calcs (2 new metrics); case QC panel + run page + `/admin/checklists` + `/dev/quality`; default Norwegian templates (Leveringssjekk, Kalibrering). (5) **Case intake UX** — reg-number/phone search with existing customer + vehicle detection and a fast create-and-open path (`/cases/new`), reusing existing services (no domain/RBAC/tenancy change). (6) Dev `/dev/documents` + `/dev/quality` inspection. Gates green: unit 51/51, integration 80/80, build. Still to come: digital signatures, communication (SMS/email + customer status updates), Dev impersonation + feature flags + rebuild-projection tool, E2E coverage, then the First-Friendly-Customer go-live gate.
